...
def get_spectrogram_feature(filepath):
    ...
    # (* , N, T, 2)
    # N is the number of frequencies where STFT is applied, T is the total number of frames used,
    amag = stft.numpy();
...
def get_script(filepath, bos_id, eos_id):
    key = filepath.split('/')[-1].split('.')[0]  ######33 time sorting input
...
def _collate_fn(batch):
    ...
    max_seq_size = max_seq_sample.size(0) # 16N 으로
    max_seq_size = max_seq_size + ( 16 - (max_seq_size % 16))
